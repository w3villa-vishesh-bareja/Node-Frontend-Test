...
const TaskCard = ({
  ...
  const availableUsers =
    userDetails?.filter((user) => user.role !== "owner") || [];

  const handleAssignUser = async (assignedUserId) => {
    try {
      const response = await axios.post(
        "https://nodetraining-ny09.onrender.com/task/changeAssignedTo",
        {
          userId,
          projectId,
          taskId: task.id,
        ...
        {!isDeleteMode && (
          <div className="flex items-center space-x-2">
            ...
            <div className="relative">
              ...
              {showUserDropdown && (
                <div
                  className="absolute right-0 mt-2 w-48 bg-gray-800 rounded-lg shadow-xl 
